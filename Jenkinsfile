        ...
        stage('Build') {
            steps {
                ...
                sh '''
                    ...
                    coverage=`grep -o -P '(?<=<span class="strong">).*(?=% </span>)' build/test/coverage/HeadlessChrome\\ 74.0.3723\\ \\(Linux\\ 0.0.0\\)/lcov-report/index.html | head -2`;
                    echo "$coverage"
                    echo "$coverage[1]"
                    coverage_int=`awk -v var="$coverage[0]" 'BEGIN {print int(var)}'`
