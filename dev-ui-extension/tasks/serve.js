...
module.exports = function(grunt) {
    ...
    var useMockAPI = require('../mock-api');
    ...
    if (grunt.option('serve')) {
        ...
        app.use('/', serveIndex(path.join(process.cwd(), 'build'), {
            icons: true
        }));
        app.listen(9000);
    }
    ...
    function findProxies() {
        grunt.option.flags()
            .forEach(function(flag) {
                ...
                var keySuffix = key.substring(key.length - 3)
                    .toLowerCase();
                ...
                if (keySuffix == 'url' && valuePrefix == 'http') {
                    ...
                    console.log('grunt.option:', key, ' -> ', proxyPath);
                    console.log('Proxy:', proxyPath, ' -> ', value);
                }
            });
    }
};