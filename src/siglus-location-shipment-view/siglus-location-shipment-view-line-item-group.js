...
(function() {
    ...
    angular
        ...
        .factory('SiglusLocationShipmentViewLineItemGroup', ShipmentViewLineItemGroup);

    ShipmentViewLineItemGroup.inject = ['SiglusLocationShipmentViewLineItem', 'classExtender'];
    ...
    function ShipmentViewLineItemGroup(ShipmentViewLineItem, classExtender) {
        ...
        ShipmentViewLineItemGroup.prototype.getRemainingSoh = getRemainingSoh;
         ...
         */
        function getAvailableSoh(inDoses, lineItems) {
            var $this = this;
            return _.reduce(lineItems, function(availableSoh, lineItem) {
                var stockOnHand = $this.recalculateQuantity(
                    _.get(lineItem.shipmentLineItem, 'stockOnHand', 0), inDoses
                );
                return availableSoh + stockOnHand;
            }, 0);
        }

        function getRemainingSoh(inDoses, lineItems) {
            var remainingQuantityInPacks = this.getAvailableSoh(inDoses, lineItems) - this.getFillQuantity(lineItems);
         ...
         */
        function getFillQuantity(lineItems) {
